"""Propiedades de la normalización de unidades de dosis (amplifica la fortaleza #6).

Congela las equivalencias físicas (kg↔g, L↔mL) y la auto-consistencia tabla↔regex, y prueba
que una dosis perturbada deja de estar respaldada (no hay falsos VERDE por redondeo grosero).
"""
...
def test_equivalent_doses_share_canonical_key() -> None:
    ...
    # Coma vs punto decimal no debe romper la igualdad.
...
def test_non_equivalent_doses_differ() -> None:
    ...
    # g/L (masa) y cc/L (volumen) NO son equivalentes sin densidad: dimensiones distintas.
    assert _canonical_doses("1 g/L") != _canonical_doses("1 cc/L")


def test_units_table_matches_regex() -> None:
    # Auto-consistencia: toda unidad con factor declarado debe ser reconocida por el regex de dosis
    # (un desajuste tabla↔regex haría que una dosis válida pasara como "número suelto").
...
def test_perturbed_dose_becomes_unsupported() -> None:
    # Una dosis perturbada ~+12% respecto a la fuente NO debe quedar respaldada.
